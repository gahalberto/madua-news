...
# Script final para execução do scraper
# Este script executa o scraper Python e depois os scripts TypeScript
...
# Instalar ts-node se não estiver instalado
if ! command -v ./node_modules/.bin/ts-node &> /dev/null; then
    echo "Instalando ts-node localmente..." | tee -a "$LOG_FILE"
    npm install --save-dev ts-node typescript @types/node
fi

# Executar usando o ts-node local
echo "Executando import_articles.ts..." | tee -a "$LOG_FILE"
./node_modules/.bin/ts-node import_articles.ts "$OUTPUT_FILE" 2>&1 | tee -a "$LOG_FILE"

# Processar artigos
echo "Processando artigos..." | tee -a "$LOG_FILE"
./node_modules/.bin/ts-node process_all_articles.ts 2>&1 | tee -a "$LOG_FILE"

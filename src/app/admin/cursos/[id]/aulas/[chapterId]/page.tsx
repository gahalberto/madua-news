...
export default function EditChapterPage({ params }: { params: { id: string; chapterId: string } }) {
  ...
  const [uploadedFiles, setUploadedFiles] = useState<ChapterFile[]>([]);
  const [currentTab, setCurrentTab] = useState<"basic" | "content" | "files">("basic");
  const [filesToDelete, setFilesToDelete] = useState<string[]>([]);
  
  const [formData, setFormData] = useState<Omit<Chapter, 'downloadFiles'>>({
    id: chapterId,
    title: "",
    description: "",
    contentType: "VIDEO",
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const chapterResponse = await fetch(`/api/courses/${courseId}/chapters/${chapterId}`);
        ...
        setFormData({
          ...
          contentType: chapterData.contentType || "VIDEO",
          ...
          position: chapterData.position || 1,
        });
        
        // Preencher a lista de arquivos
        if (chapterData.downloadFiles && chapterData.downloadFiles.length > 0) {
          setUploadedFiles(chapterData.downloadFiles);
        }
      } catch (error) {
  ...
  const handleRemoveFile = (index: number) => {
    ...
    // Se o arquivo tem um ID, adicione-o à lista de arquivos para excluir
    if (file.id) {

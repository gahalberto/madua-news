...
export async function generateMetadata({ params }: { params: { categoria: string } }): Promise<Metadata> {
  ...
  return {
    title: `${data.category.name} | Blog do Clube do Rabino`,
    description: `Artigos sobre ${data.category.name}. Conteúdo educacional de qualidade para impulsionar sua carreira.`,
    openGraph: {
      title: `${data.category.name} | Blog do Clube do Rabino`,
      description: `Artigos sobre ${data.category.name}. Conteúdo educacional de qualidade para impulsionar sua carreira.`,
      type: 'website',
    ...
    alternates: {
      canonical: `https://clubedorabino.com/blog/categoria/${encodeURIComponent(data.category.name.toLowerCase())}`,
    },
  };
}
